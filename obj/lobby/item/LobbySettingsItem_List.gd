extends Control
class_name LobbySettingsItemList


const Highlight = preload('res://obj/lobby2/item/LobbySettingsItem_highlighted.theme')

const Number = preload('res://obj/lobby2/item/NumberFont.tres')
const NumberHighlight = preload('res://obj/lobby2/item/NumberFont_highlighted.tres')

const Prompt = preload('res://obj/lobby2/item/LobbySettingsItem_Prompt.theme')
const PromptHighlight = preload('res://obj/lobby2/item/LobbySettingsItem_Prompt_highlighted.theme')

const L = preload('res://obj/lobby2/item/triangle_l.svg')
const LHighlight = preload('res://obj/lobby2/item/triangle_l_highlight.svg')

const R = preload('res://obj/lobby2/item/triangle_r.svg')
const RHighlight = preload('res://obj/lobby2/item/triangle_r_highlight.svg')
...
func _arrange_buttons():
	for button in [$Value/L, $Value/R]:
		...
		_sprite.position = button.rect_size / 2
		
		var sprite_size = _sprite.get_node('Sprite').texture.get_size()
		var y_scale = 1
		
		if sprite_size.y > button.rect_size.y:
			y_scale = button.rect_size.y / sprite_size.y
			_sprite.scale = Vector2(y_scale, y_scale)
		
		if button.name == 'L':
			_sprite.position.x \
				= button.rect_size.x \
				- (sprite_size.x * y_scale) \
				- 10
		else:
			_sprite.position.x \
				= (sprite_size.x * y_scale) \
...
func activate():
	...
	if values.size() > 1:
		for button in [$Value/L, $Value/R]:
			...
			_sprite.get_node('Sprite/Anim1').play('Wave')
			if button.name == 'L':
				_sprite.get_node('Sprite').texture = LHighlight
			else:
				_sprite.get_node('Sprite').texture = RHighlight
